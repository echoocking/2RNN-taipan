...
weights = {
    'h1': tf.Variable(tf.random_uniform([encoder_hidden_units*2, n_hidden_1],-1,1)),
...
pred = multilayer_perceptron(encoder_final_state_c, weights, biases)
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            _, c = sess.run([optimizer, cost], feed_dict={x_shrt_term: batch_x,  encoder_inputs_length: len_x,
                                                          y: batch_y})
            #print (sess.run(ff_input, feed_dict={x_shrt_term: batch_x}))
            #break
        ...
        print("Epoch %d  -- Accuracy: %f", (epoch,accuracy.eval({x_shrt_term: test_x,encoder_inputs_length:len_test, y: test_y})))